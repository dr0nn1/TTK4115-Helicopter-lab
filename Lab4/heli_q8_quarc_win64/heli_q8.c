...
void heli_q8_output0(void)             /* Sample time: [0.0s, 0.0s] */
{
  ...
  if (rtmIsMajorTimeStep(heli_q8_M)) {
    ...
    /* S-Function Block: heli_q8/Heli 3D/HIL Read Encoder Timebase (hil_read_encoder_timebase_block) */
    {
      ...
      if (result < 0) {
        ...
        rtb_Add_k = heli_q8_DW.HILReadEncoderTimebase_Buffer[2];
      }
    }
     ...
     */
    heli_q8_B.Add = heli_q8_P.ElevationCounttorad_Gain * rtb_Add_k +
  ...
  if (rtmIsMajorTimeStep(heli_q8_M)) {
    ...
    if (heli_q8_B.RateTransitiony > heli_q8_P.DeadZoney_End) {
      rtb_Add_k = heli_q8_B.RateTransitiony - heli_q8_P.DeadZoney_End;
    } else if (heli_q8_B.RateTransitiony >= heli_q8_P.DeadZoney_Start) {
      rtb_Add_k = 0.0;
    } else {
      rtb_Add_k = heli_q8_B.RateTransitiony - heli_q8_P.DeadZoney_Start;
    }
     ...
     *  Gain: '<S7>/Gain: y'
     */
    heli_q8_B.Joystick_gain_y = heli_q8_P.Gainy_Gain * rtb_Add_k *
  ...
  if (rtmIsMajorTimeStep(heli_q8_M)) {
    ...
    if (heli_q8_B.RateTransitionx > heli_q8_P.DeadZonex_End) {
      rtb_Add_k = heli_q8_B.RateTransitionx - heli_q8_P.DeadZonex_End;
    } else if (heli_q8_B.RateTransitionx >= heli_q8_P.DeadZonex_Start) {
      rtb_Add_k = 0.0;
    } else {
      rtb_Add_k = heli_q8_B.RateTransitionx - heli_q8_P.DeadZonex_Start;
    }
     ...
     *  Gain: '<S7>/Gain: x'
     */
    heli_q8_B.Joystick_gain_x = heli_q8_P.Gainx_Gain * rtb_Add_k *
   ...
   */
  tmp[0] = heli_q8_B.Integrator[0];
  tmp[1] = heli_q8_B.Integrator[1];
  tmp[2] = heli_q8_B.Integrator[3];
  tmp[3] = heli_q8_X.Integrator1_CSTATE;
  tmp[4] = heli_q8_X.Integrator_CSTATE_h;
  ...
  for (i = 0; i < 2; i++) {
    ...
    rtb_Sum[i] = heli_q8_B.Gain1[i] - u0;
  }
  ...
  if (rtmIsMajorTimeStep(heli_q8_M)) {
    ...
    /* S-Function (stream_formatted_write_block): '<S4>/Stream Formatted Write' */
    {
      ...
      if (rtb_StreamCall1_o1 != NULL) {
        result = stream_print_utf8_char_array(*rtb_StreamCall1_o1,
          ...
          "%c\n"
          , (char) heli_q8_P.Constant1_Value_b
          );
        if (result > 0) {
    ...
    for (i = 0; i < 3; i++) {
      ...
      heli_q8_B.euler_rates[i] += tmp_0[i] * tmp_1[0];
      heli_q8_B.euler_rates[i] += tmp_0[i + 3] * tmp_1[1];
      heli_q8_B.euler_rates[i] += tmp_0[i + 6] * tmp_1[2];
    }
  }
   ...
   *  Sum: '<S1>/Add'
   */
  u0 = (heli_q8_B.Add_h - rtb_Sum[1]) * heli_q8_P.Frontgain_Gain;
   ...
   *  Sum: '<S1>/Subtract'
   */
  u0 = (heli_q8_B.Add_h + rtb_Sum[1]) * heli_q8_P.Backgain_Gain;
  ...
  if (rtmIsMajorTimeStep(heli_q8_M)) {
     ...
     *  Product: '<S9>/Product1'
     */
    rtb_Add_k = rtb_Gain2[1] * rtb_Gain2[1] + rtb_Gain2[2] * rtb_Gain2[2];

    /* Sqrt: '<S9>/Sqrt' */
    rtb_Sqrt = sqrt(rtb_Add_k);
    ...
    switch (rtAction) {
     case 0:
       ...
       */
      heli_q8_B.Merge_d[0] = heli_q8_B.Merge;
      heli_q8_B.Merge_d[1] = heli_q8_B.euler_rates[0];
      heli_q8_B.Merge_d[2] = heli_q8_B.Merge1;
      heli_q8_B.Merge_d[3] = heli_q8_B.euler_rates[1];
      heli_q8_B.Merge_d[4] = heli_q8_B.euler_rates[2];
      if (rtmIsMajorTimeStep(heli_q8_M)) {
     ...
     case 1:
       ...
       */
      for (i = 0; i < 5; i++) {
        heli_q8_B.Merge_d[i] = heli_q8_P.Constant1_Value[i];
      }
  ...
  for (i = 0; i < 5; i++) {
    ...
    tmp[i] = 0.0;
    for (i_0 = 0; i_0 < 5; i_0++) {
      tmp[i] += heli_q8_P.C1[5 * i_0 + i] * heli_q8_B.Integrator[i_0];
    }

    /* End of Gain: '<S8>/Gain4' */

    /* Sum: '<S8>/Sum1' incorporates:
     *  Gain: '<S8>/Gain5'
     */
    tmp_2[i] = heli_q8_B.Merge_d[i] - tmp[i];
  }
  ...
  for (i = 0; i < 5; i++) {
     ...
     */
    tmp_3[i] = 0.0;
    tmp_3[i] += heli_q8_P.B1[i] * rtb_Sum[0];
    tmp_3[i] += heli_q8_P.B1[i + 5] * rtb_Sum[1];
    ...
    for (i_0 = 0; i_0 < 5; i_0++) {
       ...
       *  Gain: '<S8>/Gain1'
       */
      u0 += heli_q8_P.A1[5 * i_0 + i] * heli_q8_B.Integrator[i_0];

      /* Gain: '<S8>/Gain5' incorporates:
       *  Sum: '<S8>/Sum'
       */
      tmp[i] += heli_q8_P.L[5 * i_0 + i] * tmp_2[i_0];
    }

    /* Sum: '<S8>/Sum' incorporates:
     *  Gain: '<S8>/Gain1'
     */
    heli_q8_B.Sum[i] = (tmp[i] + tmp_3[i]) + u0;
  }

  /* Sum: '<Root>/Sum1' */
  heli_q8_B.Sum1 = heli_q8_B.Integrator[3] - heli_q8_B.Joystick_gain_y;

  /* Sum: '<Root>/Sum2' */
  heli_q8_B.Sum2 = heli_q8_B.Integrator[0] - heli_q8_B.Joystick_gain_x;
  if (rtmIsMajorTimeStep(heli_q8_M)) {
    /* ToFile: '<Root>/To File' */
    if (rtmIsMajorTimeStep(heli_q8_M)) {
      {
        if (!(++heli_q8_DW.ToFile_IWORK.Decimation % 1) &&
          ...
          if (fp != (NULL)) {
            ...
            if (fwrite(u, sizeof(real_T), 2, fp) != 2) {
              rtmSetErrorStatus(heli_q8_M, "Error writing to MAT-file y.mat");
              return;
            }
...
void heli_q8_derivatives(void)
{
  ...
  for (i = 0; i < 5; i++) {
    _rtXdot->Integrator_CSTATE[i] = heli_q8_B.Sum[i];
  }
  ...
  _rtXdot->Integrator_CSTATE_h = heli_q8_B.Sum1;
}
...
void heli_q8_initialize(void)
{
  ...
  {
    FILE *fp = (NULL);
    char fileName[509] = "y.mat";
    if ((fp = fopen(fileName, "wb")) == (NULL)) {
      rtmSetErrorStatus(heli_q8_M, "Error creating .mat file y.mat");
      return;
    }

    if (rt_WriteMat4FileHeader(fp,2,0,"ans")) {
      rtmSetErrorStatus(heli_q8_M, "Error writing mat file header to file y.mat");
      return;
    }
...

  {
    ...
    heli_q8_X.Integrator_CSTATE_h = heli_q8_P.Integrator_IC_o;
...
void heli_q8_terminate(void)
{
  ...
  /* Terminate for ToFile: '<Root>/To File' */
  {
    ...
    if (fp != (NULL)) {
      char fileName[509] = "y.mat";
      if (fclose(fp) == EOF) {
        rtmSetErrorStatus(heli_q8_M, "Error closing MAT-file y.mat");
        return;
      }

      if ((fp = fopen(fileName, "r+b")) == (NULL)) {
        rtmSetErrorStatus(heli_q8_M, "Error reopening MAT-file y.mat");
        return;
      }

      if (rt_WriteMat4FileHeader(fp, 2, heli_q8_DW.ToFile_IWORK.Count, "ans")) {
        rtmSetErrorStatus(heli_q8_M,
                          "Error writing header for ans to MAT-file y.mat");
      }
...
RT_MODEL_heli_q8_T *heli_q8(void)
{
  ...
  /* External mode info */
  heli_q8_M->Sizes.checksums[0] = (289636079U);
  heli_q8_M->Sizes.checksums[1] = (2867893016U);
  heli_q8_M->Sizes.checksums[2] = (2637148078U);
  heli_q8_M->Sizes.checksums[3] = (3388461019U);

  {
    ...
    systemRan[3] = (sysRanDType *)&heli_q8_DW.IfActionSubsystem1_SubsysRanBC;
    systemRan[4] = (sysRanDType *)&heli_q8_DW.IfActionSubsystem2_SubsysRanBC;
  ...
  heli_q8_M->Sizes.numBlocks = (106);  /* Number of blocks */
  heli_q8_M->Sizes.numBlockIO = (29);  /* Number of block outputs */
  heli_q8_M->Sizes.numBlockPrms = (537);/* Sum of parameter "widths" */
  return heli_q8_M;
}
